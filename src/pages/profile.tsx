...
import { useGetUserProfile, useUpdateUserProfile, useGetUserAddresses, useAddAddress } from "@/lib/api-client";
...
export default function Profile() {
  ...
  const { data: profile, isLoading } = useGetUserProfile({ query: { enabled: !!token } });
  const { data: addresses } = useGetUserAddresses({ query: { enabled: !!token } });
  ...
  const { mutate: addAddress, isPending: isAddingAddr } = useAddAddress({
    mutation: {
      onSuccess: () => {
