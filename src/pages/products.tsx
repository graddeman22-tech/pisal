...
export default function Products() {
  ...
  const [, setDebounce] = useState<ReturnType<typeof setTimeout> | null>(null);
  const handleSearchChange = (value: string) => {
    setSearch(value);
    setDebounce((prev) => {
      if (prev) clearTimeout(prev);
      return setTimeout(() => setDebouncedSearch(value), 500);
    });
  };
        ...
        <div className="space-y-2">
          ...
          {categories?.map(c => (
            <label key={c.id} className="flex items-center gap-3 cursor-pointer">
              <input type="radio" name="category" checked={category === c.slug} onChange={() => setCategory(c.slug)} className="accent-primary w-4 h-4" />
              <span className={category === c.slug ? "text-primary font-medium" : "text-muted-foreground"}>{c.name} ({c.productCount})</span>
            </label>
          ))}
        </div>
      </div>
