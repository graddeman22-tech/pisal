...
app.get("/api/products", async (req: any, res: any) => {
  try {
    ...
    if (category) conditions.push(eq(categoriesTable.slug, category as string));
  ...
  try {
    ...
    if (quantity <= 0) {
      await db
        ...
        .where(
          and(eq(cartItemsTable.id, cleanId), eq(cartItemsTable.userId, safeNum(user.id))),
        );
    } else {
      await db
        .update(cartItemsTable)
...
app.post("/api/wishlist/:productId", requireAuth, async (req: any, res: any) => {
  try {
    ...
    const existing = await db
      .select()
      .from(wishlistTable)
      .where(and(eq(wishlistTable.userId, safeNum(user.id)), eq(wishlistTable.productId, productId)));
    if (!existing.length)
...
app.post("/api/orders", requireAuth, async (req: any, res: any) => {
  try {
    ...
    if (couponCode) {
      const stringCoupon = safeStr(couponCode).toUpperCase();
      const coupons = await db
        .select()
        .from(couponsTable)
        .where(eq(couponsTable.code, stringCoupon as string));
...
app.get("/api/orders/:id", requireAuth, async (req: any, res: any) => {
  try {
    ...
    const orders = await db
      .select()
      .from(ordersTable)
      .where(and(eq(ordersTable.id, cleanId), eq(ordersTable.userId, safeNum(user.id))));
    if (!orders.length) {
...
app.post("/api/coupons/validate", async (req: any, res: any) => {
  try {
    const queryCode = req.body.code ? safeStr(req.body.code) : (req.query.code ? safeStr(req.query.code) : "");
    ...
    const stringQueryCode = safeStr(queryCode).toUpperCase();
    const coupons = await db
      .select()
      .from(couponsTable)
      .where(eq(couponsTable.code, stringQueryCode as string));
    const coupon = coupons[0];
    if (!coupon) {
      res.json({ valid: false, message: "Invalid coupon code" });
      return;
...
app.get("/api/admin/orders", requireAdmin, async (req: any, res: any) => {
  try {
    ...
    const conditions = [];
    if (status) {
      const stringStatus = safeStr(status);
      conditions.push(eq(ordersTable.status, stringStatus as string));
    }
    const whereCondition = conditions.length > 0 ? and(...conditions) : undefined;
    ...
    res.json(adminOrders.map(formatOrder));
  } catch (err) {
    console.error(err);
