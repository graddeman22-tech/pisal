...
function FlipkartCard({ product }: { product: any }) {
  ...
  const handleWish = (e: React.MouseEvent) => {
    e.preventDefault();
    if (!user) return setAuthModalOpen(true);
    isWishlisted ? removeWish({ product_id: product.id }) : addWish({ 
      user_id: user.id,
      product_id: product.id 
    });
  };
      ...
      <Link href={`/products/${product.id}`} className="group block bg-card border border-border/40 rounded-2xl overflow-hidden hover:shadow-lg hover:-translate-y-0.5 transition-all duration-250">
        ...
        <div className="relative aspect-[4/5] bg-muted overflow-hidden">
          ...
          <div className="absolute inset-x-0 bottom-0 translate-y-full group-hover:translate-y-0 transition-transform duration-300 p-2">
            <button onClick={handleCart} disabled={isPending || !product.in_stock}
              ...
              {isPending ? "Adding…" : product.in_stock ? "Quick Add" : "Out of Stock"}
            </button>
          </div>
        </div>
        ...
        {/* Info */}
        <div className="p-3">
          <p className="text-[0.6rem] font-bold text-primary uppercase tracking-widest mb-1">{product.category || "Spices"}</p>
          ...
          <div className="flex items-center gap-1.5 flex-wrap mb-2.5">
            ...
            {savings > 0 && <span className="text-xs font-bold text-green-600">Save ₹{product.original_price - product.price}</span>}
          </div>
          <button onClick={handleCart} disabled={isPending || !product.in_stock}
            ...
            <ShoppingBag className="w-3.5 h-3.5" />
            {isPending ? "Adding…" : product.in_stock ? "Add to Cart" : "Out of Stock"}
          </button>
        </div>
      </Link>
    </motion.div>
...
export default function Home() {
  ...
  const filtered = products.filter((p: any) => {
    const matchCat = !activeCategory || p.category_slug === activeCategory || p.categorySlug === activeCategory;
  ...
  const featuredProducts = products.filter((p: any) => p.is_bestseller || p.isBestseller || p.rating >= 4.8).slice(0, 4);
